...
(function () {
    ...
    function Modelfactory(Modelvalidation) {
        ...
        return function innerModelfactory(model, selectedObjects, edgeAddedCallback, nodeRemovedCallback, edgeRemovedCallback, checkboxCheckedCallback) {
            ...
            modelservice.nodes = {
                ...
                getConnectorsByName: function (node) {
                    return node.connectors.filter(function (connector) {
                        return connector.name
                    });
                },

                getConnectorIsChecked: function (node) {
                    return node.connectors.filter(function (connector) {
                        return connector.isChecked;
                    });
                },
                ...
                checkBoxClick: function (connectorId,checkedStatus) {                    
                    modelservice.checkboxCheckedCallback(connectorId, checkedStatus);
                },
          ...
          (edge) {
            ...
            modelservice.registerCallbacks = function (edgeAddedCallback, nodeRemovedCallback, edgeRemovedCallback, checkboxCheckedCallback) {
                ...
                modelservice.checkboxCheckedCallback = checkboxCheckedCallback;
            };
...
(function () {
    ...
    function Joindrawingservice() {

        this.getCircleCyAttribute = function (pt1, pt2) {

            var CyAddribute = null;
 
            if (pt1.y < pt2.y) {

                CyAddribute = ((pt2.y - pt1.y) / 2) + pt1.y;
            } else {
        ...
        this.getCircleCxAttribute = function (pt1, pt2) {

            var CxAddribute = null;

            if (pt1.x < pt2.x) {

                CxAddribute = ((pt2.x - pt1.x) / 2) + pt1.x;
            } else {
    ...
    Joindrawingservice.$inject = [];

    angular
        .module('flowchart')
        .service('Joindrawingservice', Joindrawingservice );
...
(function () {
    ...
    function Edgedraggingfactory(Modelvalidation, flowchartConstants, Edgedrawingservice, Joindrawingservice) {
    ...
    Edgedraggingfactory.$inject = ["Modelvalidation", "flowchartConstants", "Edgedrawingservice", "Joindrawingservice"];
...
(function () {
    ...
    function fcCheckbox(flowchartConstants) {
        return {
            restrict: 'A',
            templateUrl: "flowchart/column.html",
            replace: true,
            transclude: true,
            link: function (scope, element)
            {

                scope.toggleMe = function (connectorId, checkedStatus)
                {
                    scope.connector.isChecked = !checkedStatus;
                    scope.modelservice.nodes.checkBoxClick(connectorId, scope.connector.isChecked);
                }
            }
        };
    }
    fcCheckbox.$inject = ["flowchartConstants"];

    angular
        .module('flowchart')
        .directive('fcCheckbox', fcCheckbox);
...
(function () {
    ...
    function venDiag() {
        ...
        return {
            ...
            templateUrl: "flowchart/Vendiagram.html",


            //template: '<div><div class="outer1" ng-attr-style="position: absolute; top: 546px; left: 716px;"></div><div class="outer2" ng-attr-style="position: absolute; top: 546px; left: 741px;"></div></div>',

            link: function (scope, elem, attrs) {
                var status;
                scope.toggle = function () {
                    if (elem.hasClass('nodecheckedAll')) {
                        elem.removeClass('nodecheckedAll');
                        status = false;
                    }
                    else {
                        elem.addClass('nodecheckedAll');
                        status = true;
                    }

                    angular.forEach(scope.modelservice.nodes.getConnectorsByName(scope.node), function (connector) {
                        connector.isChecked = status;
...
(function () {
    ...
    function fcCheckboxAll(flowchartConstants) {
        return {
            restrict: 'E',
            replace: true,
            transclude: true,
            template: '<span class="nodecheckboxAll" ng-class="{nodecheckedAll}"  ng-click="toggle()"></span>',
    ...
    fcCheckboxAll.$inject = ["flowchartConstants"];

    angular
        .module('flowchart')
        .directive('fcCheckboxAll', fcCheckboxAll);
...
(function () {
    ...
    function canvasController($scope, Mouseoverfactory, Nodedraggingfactory, Modelfactory, Edgedraggingfactory, Edgedrawingservice, Joindrawingservice, FlowchartCanvasService) {
        ...
        $scope.modelservice = Modelfactory($scope.model, $scope.selectedObjects, $scope.userCallbacks.edgeAdded || angular.noop, $scope.userCallbacks.nodeRemoved || angular.noop, $scope.userCallbacks.edgeRemoved || angular.noop, $scope.userCallbacks.checkboxChecked || angular.noop);
        ...
        $scope.getCircleCxAttribute = Joindrawingservice.getCircleCxAttribute;
        $scope.getCircleCyAttribute = Joindrawingservice.getCircleCyAttribute;
    }
    canvasController.$inject = ["$scope", "Mouseoverfactory", "Nodedraggingfactory", "Modelfactory", "Edgedraggingfactory", "Edgedrawingservice", "Joindrawingservice", "FlowchartCanvasService"];
...
(function (module) {
    ...
    module.run(['$templateCache', function ($templateCache) {
        $templateCache.put('flowchart/canvas.html',
            ...
            '    <ven-diag ng-repeat="edge in model.edges">\n' +
            ...
            ' <div\n' +
                ...
                ' <div class="innerNode">\n' +
                    ' <table>\n' +
                     ...
                     ' <tr>\n' +
            '<th colspan="2"><p>{{node.name}}</p></th>\n' +
            '<th><fc-checkbox-all ng-transclude>&#10038;</fc-checkbox-all></th>\n' +
                    ' </tr>\n' +

                    ' <tr fc-checkbox \n' +
                    ' ng-repeat="connector in modelservice.nodes.getConnectorsByName(node)"></tr>\n' +
                    ' </table>\n' +
            ' </div>\n' +
                ' <div class="fc-nodedelete" ng-click="modelservice.nodes.delete(node)">&times;</div>\n' +
            ' </div>\n' +
            '');
    }]);
})();
...
(function (module) {
    ...
    module.run(['$templateCache', function ($templateCache) {
        $templateCache.put('flowchart/Vendiagram.html',
            ...
            //' class="edge-endpoint" \n' +
            //' r= "13" \n' +
            //' ng- attr - cx="{{getCircleCxAttribute(modelservice.edges.sourceCoord(edge), modelservice.edges.destCoord(edge))-9}}"\n' +
            //' ng-attr-cy="{{getCircleCyAttribute(modelservice.edges.sourceCoord(edge), modelservice.edges.destCoord(edge))}}">\n' +
            //' </circle> \n' +
            ...
            ' <div>\n' +
                ...
                ' top: {{getCircleCyAttribute(modelservice.edges.sourceCoord(edge), modelservice.edges.destCoord(edge))-20}}px;\n' +
                ' left: {{getCircleCxAttribute(modelservice.edges.sourceCoord(edge), modelservice.edges.destCoord(edge))-35}}px;">\n' +
                '</div>\n' +
                ...
                ' ng-attr-style="position: absolute; \n' +
                ' top: {{getCircleCyAttribute(modelservice.edges.sourceCoord(edge), modelservice.edges.destCoord(edge))-20}}px;\n' +
                ' left: {{getCircleCxAttribute(modelservice.edges.sourceCoord(edge), modelservice.edges.destCoord(edge))-5}}px;">\n' +
                '</div>\n' +
            '</div>\n' +


            //'<div id="midw1">\ n' +
            //'<div id="mid1" class="mid"></div>\ n' +
            //'</div>\ n' +
...
(function (module) {
    ...
    module.run(['$templateCache', function ($templateCache) {
        $templateCache.put('flowchart/column.html',
            ' <tr>\n' +
            ' <td><div fc-magnet fc-connector></div></td>\n' +
            ' <td><p>{{connector.name}}</p></td>\n' +
            //ng - class="{nodeUnchecked: !{{connector.isChecked}}}"
            ' <td><span class="nodecheckbox"   ng-click="toggleMe(connector.id,connector.isChecked)">{{connector.isChecked ? "&#10004;":""}}</span></td>\n' +
            ' </tr>\n' +
            '');
    }]);
})();
