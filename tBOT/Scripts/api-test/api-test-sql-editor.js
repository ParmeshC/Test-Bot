var SqlEditorApp = angular.module('api.test.sql.editor', ['flowchart'])
...
SqlEditorApp.controller('SqlEditorCtrl', function AppCtrl($scope, $log, $http, $uibModal, $filter, prompt, QueryBuilderFactory, Modelfactory, flowchartConstants, apiTestBroadcastService) {
    var vm = this;


    $scope.$on('handleShowSqlEditorBroadcast', function () {
        $scope.ShowSqlEditor = apiTestBroadcastService.sharedObjects.ShowSqlEditor;
    });
...
SqlEditorApp.factory('QueryBuilderFactory', function ($http) {
    return {
...
SqlEditorApp.controller('InstanceController', function ($scope, $uibModalInstance, items) {
    $scope.items = items;
    $scope.mydata;
